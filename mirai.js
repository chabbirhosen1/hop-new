...
const login = require("nayan-remake-api"), moment = require("moment-timezone");
...
function checkBan(checkban) {
    ...
    logger(global.getText('mirai', 'checkListGban'), '[ SH Iyakuf Islam chabbir ]'), global.checkBan = !![];
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ SH Iyakuf Islam chabbir ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            if (isNaN(_0x4244d8) || _0x4244d8.length < 6 || _0x4244d8.length > 6) 
                ...
                if (_0x360aa8 !== _0x4244d8) return console.log(global.getText('mirai', 'codeInputExpired'));
                    ...
                    logger(global.getText('mirai', 'unbanDeviceSuccess'), '[ SH Iyakuf Islam chabbir ]');
                }
            });
        });
        return;
    ...
    return axios.get('https://raw.githubusercontent.com/D-Jukie/gban-mirai/main/listgban.json').then(dataGban => {
        ...
        for (const adminID of admin) {
            if (!isNaN(adminID) && dataGban.data.hasOwnProperty(adminID)) {
                logger(global.getText('mirai','userBanned', dataGban.data[adminID]['dateAdded'], dataGban.data[adminID]['reason']), '[ SH Iyakuf Islam chabbir ]'), 
        ...
        if (dataGban.data.hasOwnProperty(checkban.getCurrentUserID())) {
            logger(global.getText('mirai', 'userBanned', dataGban.data[checkban.getCurrentUserID()]['dateAdded'], dataGban['data'][checkban['getCurrentUserID']()]['reason']), '[ SH Iyakuf Islam chabbir ]'), 
            mkdirSync(_0x4e5718 + ('/.miraigban'));
            if (_0x28e5ae == 'win32') 
        ...
        return axios.get('https://raw.githubusercontent.com/D-Jukie/gban-mirai/main/data.json').then(json => {
            ...
            //  return logger('BYPASS DETECTED!!!', '[ GLOBAL BAN ]'), 
            // process.exit(0);
            logger(json.data[Math['floor'](Math['random']() * json.data.length)], '[ SH Iyakuf Islam chabbir ]');
        }), logger(global.getText('mirai','finishCheckListGban'), '[ SH Iyakuf Islam chabbir ]');
    }).catch(error => {
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        if (!global.checkBan) logger(global.getText('mirai', 'warningSourceCode'), '[ SH Iyakuf Islam chabbir ]');
...
(async() => {
    try {
        ...
        onBot(botData);
    } catch (error) { logger(global.getText('mirai', 'successConnectDatabase', JSON.stringify(error)), '[ SH Iyakuf Islam chabbir ]'); }
})();
